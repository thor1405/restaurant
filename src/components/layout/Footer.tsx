...
export default async function Footer() {
  ...
  return (
    <footer className="bg-[#E5E0D8] border-t border-black/10 pt-20 pb-10 text-black mt-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16">
          ...
          <div className="flex flex-col">
            ...
            <div className="flex gap-4">
              <a href={getFormattedUrl(settings.igLink)} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/20 flex items-center justify-center text-black/80 hover:text-(--color-accent) hover:border-(--color-accent) transition-colors text-xs font-medium">
                IG
              </a>
              <a href={getFormattedUrl(settings.fbLink)} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/20 flex items-center justify-center text-black/80 hover:text-(--color-accent) hover:border-(--color-accent) transition-colors text-xs font-medium">
                FB
              </a>
              <a href={getFormattedUrl(settings.xLink)} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/20 flex items-center justify-center text-black/80 hover:text-(--color-accent) hover:border-(--color-accent) transition-colors text-xs font-medium">
                X
              </a>
            </div>
          </div>
          ...
          <div className="flex flex-col">
            ...
            <ul className="flex flex-col gap-4">
              {['Menu', 'Our Story', 'Gallery', 'Events'].map((link) => (
                <li key={link}>
                  <Link
