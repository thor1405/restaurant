  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-8">
        <h1 className="font-heading text-3xl text-white">Slot Settings</h1>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="col-span-1">
          <div className="bg-[#1C1C1C] rounded-lg border border-white/10 p-6">
            <h2 className="text-xl text-white font-heading mb-6">Add / Update Slot</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-white/70 text-xs tracking-widest uppercase mb-2">Time (HH:MM)</label>
                <input 
                  type="time" 
                  required 
                  value={time} 
              ...
              <div>
                <label className="block text-white/70 text-xs tracking-widest uppercase mb-2">Max Guests Capacity</label>
                <input 
                  type="number" 
                  required 
                  min="1"
        ...
        <div className="col-span-1 lg:col-span-2">
          <div className="bg-[#1C1C1C] rounded-lg border border-white/10 overflow-hidden">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={3} className="p-4 text-white/50">Loading...</td></tr>
                ) : slots.length === 0 ? (
                  <tr>
                  ...
                  slots.map((slot) => (
                    <tr key={slot._id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                      <td className="p-4">
                        <div className="text-(--color-accent) text-lg font-medium">{slot.time}</div>
                      </td>
                      <td className="p-4">
                        <div className="text-white">{slot.maxGuests} guests</div>
                      </td>
